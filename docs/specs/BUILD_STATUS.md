...
| P07 | Frontend (deck.gl + MapLibre) | done ✅ | React+Vite+TS, deck.gl interleaved overlay on MapLibre; design tokens + ramp + fonts; tickStore (typed arrays) + appStore + REST/WS client. **WIRED TO LIVE BACKEND** — renders the real 18,190-edge graph from `/edges`, recolored by real engine pressures from `/demo/run`; Before/After from real summaries; copilot from `/copilot/plan`; transit from `/transit`. No hardcoded sim data (deleted `data/demo.ts`). Browser-verified end-to-end against the running API. tsc+build clean, 12 vitest. PR: FLO-12. |
