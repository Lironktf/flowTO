...
def _graph_state():
    ...
    g.add_edge(0, 1, key=0, **schema.make_edge(
        edge_id="dufferin", from_node=0, to_node=1, road_name="Dufferin Street",
        road_class="secondary", length_m=500.0, speed_kmh=50.0, lanes=2.0,
        capacity=1200.0, base_time_min=0.6, one_way=False, geometry=[[43.63, -79.41], [43.63, -79.40]]))
    g.add_edge(1, 2, key=0, **schema.make_edge(
        edge_id="strachan", from_node=1, to_node=2, road_name="Strachan Avenue",
        road_class="tertiary", length_m=400.0, speed_kmh=40.0, lanes=1.0,
        capacity=800.0, base_time_min=0.5, one_way=False, geometry=[[43.63, -79.40], [43.64, -79.40]]))
    return AppState.from_graph(g, [{"origin": 0, "destination": 2, "trips": 900.0}],
                               weather="clear", time_context={"hour": 17})
...
def test_plan_attaches_advisory_for_major_arterial(monkeypatch):
    ...
    good = ToolCall(tool="preview_intervention",
                    interventions=[{"op": "close_edge", "edge_id": "strachan"}]).model_dump_json()
